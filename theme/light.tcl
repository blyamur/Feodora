...
namespace eval ttk::theme::spring-noon-light {
    ...
    ttk::style theme create spring-noon-light -parent clam -settings {
        ...
        ttk::style layout Url.TButton {
            UrlButton.button -children {
                UrlButton.padding -children {
                    UrlButton.label -side left -expand 1
                } 
            }
        }
        ...
        # Url.TButton
        ttk::style configure Url.TButton -padding {8 4} -anchor center -foreground #2E3842

        ttk::style map Url.TButton -foreground \
            [list disabled #2E3842 \
                pressed #2E3842]

        ttk::style element create UrlButton.button image \
            [list $images(button-url-rest) \
                {selected disabled} $images(button-url-disabled) \
                disabled $images(button-url-disabled) \
                selected $images(button-url-rest) \
                pressed $images(button-url-pressed) \
                active $images(button-url-hover) \
            ] -border 4 -sticky nsew
